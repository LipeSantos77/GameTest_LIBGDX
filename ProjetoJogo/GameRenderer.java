...
import com.badlogic.gdx.utils.viewport.Viewport; // Importa Viewport
...
public class GameRenderer {
    ...
    private float gameOverFadeAlpha = 0f; // Para o overlay de Game Over
    private float levelCompleteFadeAlpha = 0f; // Para o overlay de Level Complete

    // --- Cores Dinâmicas por Fase ---
    private Color avalancheColor1, avalancheColor2, avalancheColor3;
    private Color trackBaseColor; // Cor base da pista (neve/gelo)
    private Color skyColor; // Cor do céu/fundo principal
    ...
    public GameRenderer(MainGame game, GameWorld world) {
        ...
        // Define as cores e carrega as texturas baseadas no nível inicial
        defineLevelColors();
        loadTextures();
    ...
    private void defineLevelColors() {
        if (world.getCurrentLevel() == 3) {
            // Cores da Neve (Fase 3 - lava)
            avalancheColor1 = new Color(1.0f, 0.75f, 0.1f, 0.95f);  // Frente - amarelo incandescente (mais quente)
            avalancheColor2 = new Color(1.0f, 0.35f, 0.05f, 0.8f);  // Meio - laranja quente
            avalancheColor3 = new Color(0.55f, 0.05f, 0.02f, 0.55f); // Fundo - vermelho escuro derretido
            trackBaseColor = new Color(0.35f, 0.0f, 0.0f, 1f); // Solo - pedra vulcânica escura (basalto)
            skyColor = new Color(0.05f, 0.0f, 0.0f, 1f);      // Céu - fumaça vulcânica escura
        }else   if (world.getCurrentLevel() == 2) {
            // Cores da Neve (Fase 2 - cave / marrom)
            avalancheColor1 = new Color(0.65f, 0.45f, 0.25f, 0.9f);  // Frente (mais opaca)
            avalancheColor2 = new Color(0.55f, 0.35f, 0.18f, 0.7f);  // Meio
            avalancheColor3 = new Color(0.45f, 0.28f, 0.15f, 0.4f);  // Fundo
            trackBaseColor = new Color(0.7f, 0.55f, 0.4f, 1f); // Solo marrom claro
            skyColor = new Color(0.25f, 0.15f, 0.1f, 1f); // Fundo Fase 2 (escuro)
        } else {
            // Cores de Neve (Fase 1 - Neve Padrão)
            ...
            trackBaseColor = new Color(0.98f, 0.98f, 0.98f, 1); // Neve para a base (branco)
            skyColor = new Color(0.6f, 0.7f, 0.8f, 1); // Fundo Fase 1 (Azul/cinza BEM claro)
        }
    }
    ...
    private void loadTextures() {
        try {
            ...
            if (world.getCurrentLevel() == 1) {
                ...
                backgroundPath = "caveBackground.jpg";
            }else{
                backgroundPath = "LavaBackround.jpeg";
            }
            if (background != null) background.dispose(); // Libera a textura antiga
            ...
            // --- Carrega textura da pedra (pode falhar se arquivo não existir) ---
            // --- Carrega textura da pedra e do zig-zag conforme a fase ---
            try {
                if (rockTexture != null) rockTexture.dispose();

                if (world.getCurrentLevel() == 3) {
                    rockTexture = new Texture(Gdx.files.internal("lava.png"));   // PEDRA DE LAVA
                } else if (world.getCurrentLevel() == 2) {
                    rockTexture = new Texture(Gdx.files.internal("pedra.png"));  // PEDRA NORMAL
                } else {
                    rockTexture = null; // Fase 1 não tem pedra
                }

            } catch (Exception ex) {
                Gdx.app.error("GameRenderer", "Erro ao carregar textura da pedra: " + ex.getMessage());
                rockTexture = null;
            }

            // --- ZIGZAG / MORCEGO / GHAST ---
            try {
                // Fase 2 = morcego normal
                // Fase 3 = ghast
                for (GameWorld.ZigZagProp p : world.getZigZagProps()) {
                    if (p.texture != null) p.texture.dispose();
                    if (world.getCurrentLevel() == 2) {
                        p.texture = new Texture(Gdx.files.internal("morcego.png"));
                    } else if (world.getCurrentLevel() == 3) {
                        p.texture = new Texture(Gdx.files.internal("ghast.png"));
                    }
                }
            } catch (Exception ex) {
                Gdx.app.error("GameRenderer", "Erro ao carregar textura do zig-zag: " + ex.getMessage());
            }
            ...
            // Texturas de placeholder para fallback
    ...
    public void render(float delta) {
        // Checa se a cor do céu/fundo precisa ser atualizada
        boolean shouldUpdateColors = false;
        if (world.getCurrentLevel() == 2 && skyColor.b > 0.4f) { // Fase 1 é mais clara
            shouldUpdateColors = true;
        } else if (world.getCurrentLevel() == 1 && skyColor.b < 0.4f) { // Fase 2 é mais escura
            shouldUpdateColors = true;
        }
        ...
        // 1. Desenhar a Pista de Neve/Gelo (Base do chão)
        shapeRenderer.setProjectionMatrix(camera.combined);
        drawTrackBase();

        // 2. Desenhar Background (Batch) - Montanha/Árvores
        game.batch.setProjectionMatrix(camera.combined);
        drawBackground();

        // 3. Desenhar a Avalanche (ondas de neve/gelo)
        drawAvalanche();
        drawWindAndDashEffects();

        // 4. Desenhar Entidades (Batch)
        game.batch.begin();

        // Desenha as pedras (somente Fase 2)
        ...
        // DESENHAR MORCEGO (ZIG-ZAG) — SEMPRE NA FRENTE DA AVALANCHE
        if (world.getCurrentLevel() >= 2) {

            // Debug opcional para confirmar que existe
            // Gdx.app.log("Renderer", "ZigZag count: " + world.getZigZagProps().size());

            for (GameWorld.ZigZagProp p : world.getZigZagProps()) {
                if (p.texture != null) {
                    game.batch.draw(p.texture, p.x, p.y, p.width, p.height);
                }
            }
        }

        // Desenha tronco (apenas na Fase 1)
        if (world.getCurrentLevel() == 1) {
            drawBot();
        }
        ...
        // 5. Desenhar UI (Batch e ShapeRenderer)
    ...
    private void updateGameOverFade(float delta) {
        if (world.isGameOver()) {
            gameOverFadeAlpha = Math.min(1.0f, gameOverFadeAlpha + delta * 1.5f); // Fade in
        } else {
            gameOverFadeAlpha = Math.max(0.0f, gameOverFadeAlpha - delta * 1.5f); // Fade out
        }
    }
    private void updateLevelCompleteFade(float delta) {
        if (world.isLevelComplete()) {
            levelCompleteFadeAlpha = Math.min(1.0f, levelCompleteFadeAlpha + delta * 1.5f); // Fade in
        } else {
            levelCompleteFadeAlpha = Math.max(0.0f, levelCompleteFadeAlpha - delta * 1.5f); // Fade out
        }
    }
    ...
    private void drawTrackBase() {
        ...
        shapeRenderer.setColor(trackBaseColor); // Usa a cor da pista base
    ...
    private void drawAvalanche() {
        ...
        for (GameWorld.SnowEffect effect : world.getSnowEffects()) {
            // pegar alpha uma vez para usar nas 3 fases
            float alpha = effect.color.a;

            if (world.getCurrentLevel() == 3) {
                // FASE 3 – partículas de LAVA / BRASA
                Color lavaFront = new Color(1f, 0.25f, 0f, alpha);     // frente (mais quente)
                Color lavaBack  = new Color(1f, 0.45f, 0.05f, alpha);  // fundo (mais amarelado)

                Color base = alpha > 0.5f ? lavaFront : lavaBack;

                // leve variação para parecer brasa vibrando
                float vary = MathUtils.random(-0.05f, 0.05f);
                Color drawColor = new Color(
                    Math.min(1f, base.r + vary),
                    Math.max(0f, base.g + vary),
                    Math.max(0f, base.b - vary),
                    alpha * 0.9f
                );
                ...
                // FASE 2 – poeira marrom
                Color base = alpha > 0.75f ? avalancheColor1 : avalancheColor2;
                Color drawColor = new Color(base.r, base.g, base.b, alpha);
                shapeRenderer.setColor(drawColor);
            } else {
                // FASE 1 – neve branca padrão
        ...
        shapeRenderer.setColor(avalancheColor3); // Usa a cor da Fase
        ...
        shapeRenderer.setColor(avalancheColor2); // Usa a cor da Fase
        for (float y = 0; y < currentAvalancheHeight; y += 2) {
        ...
        shapeRenderer.setColor(avalancheColor1); // Usa a cor da Fase
    ...
    public void dispose() {
        ...
        if (rockTexture != null) rockTexture.dispose(); // libera pedra
    }

    private Texture createPlaceholderTexture(int width, int height, Color color) {
        // Exemplo básico de implementação de placeholder.
