...
public class GameScreen implements Screen {
    ...
    public GameScreen(MainGame game, int level) {
        ...
        try {
            ...
            Gdx.app.log("GameScreen", "Texturas principais carregadas com sucesso");
        } catch (Exception e) {
        ...
        // CORREÇÃO: Carregar BotLog com tratamento robusto
        try {
            // Tentar carregar a textura primeiro para verificar se existe
            Texture troncoTexture = new Texture(Gdx.files.internal("tronco.png"));
            bot = new BotLog(troncoTexture, VIRTUAL_WIDTH + 50, getRandomPositionInTrack());
            Gdx.app.log("GameScreen", "BotLog criado com tronco.png");
        } catch (Exception e) {
            Gdx.app.error("GameScreen", "Erro ao carregar tronco.png: " + e.getMessage());
            // Fallback seguro
            try {
                bot = new BotLog("tronco.png", VIRTUAL_WIDTH + 50, getRandomPositionInTrack());
            } catch (Exception e2) {
                Gdx.app.error("GameScreen", "Erro crítico ao criar BotLog, usando fallback: " + e2.getMessage());
                bot = new BotLog();
                bot.updatePosition(VIRTUAL_WIDTH + 50, getRandomPositionInTrack());
            }
        }
    ...
    public void render(float delta) {
        ...
        // Se não estiver pausado, atualiza lógica do jogo
        if (!isPaused) {
            if (!podeLevarDano) {
            ...
            // Atualizar BotLog
            if (bot != null) {
                bot.update(delta, playerSpeedX, LARGURA_AVALANCHE, VIRTUAL_WIDTH);
            }
            ...
            if (bot != null && playerRect.overlaps(bot.rect) && podeLevarDano) {
                aplicarDano();
                bot.respawn(VIRTUAL_WIDTH, 50, 600);
            }
        ...
        // Desenhar botão de pause e overlay se pausado
        shapeRenderer.setProjectionMatrix(camera.combined);
        Gdx.gl.glEnable(GL20.GL_BLEND);
        Gdx.gl.glBlendFunc(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
