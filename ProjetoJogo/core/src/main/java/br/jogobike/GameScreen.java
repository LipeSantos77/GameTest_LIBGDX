...
public class GameScreen implements Screen {
    ...
    // Aplicar dano por tronco (colisão)
    ...
    // Aplicar dano por avalanche
    private void aplicarDanoAvalanche() {
        if (podeLevarDano) {
            vidas--;
            pontos -= 20; // -20 por avalanche
            podeLevarDano = false;
            tempoInvencivel = TEMPO_INVENCIBILIDADE;
    ...
    public void render(float delta) {
        // Sempre ouvir ESC para alternar pause
        if (Gdx.input.isKeyJustPressed(com.badlogic.gdx.Input.Keys.ESCAPE)) {
            togglePause();
        }

        // Checar toque/click no botão (sempre ouvir)
        ...
        // Desenhar background mesmo se pausado (pausa só congela lógica)
        drawBackground();

        // Desenhar avalanche e vento (efeitos visuais; nota: quando pausado estes não são atualizados,
        // mas podem continuar a ser desenhados com o último estado)
        drawAvalanche();
        drawWindEffect();
        ...
        float shownSpeed = MathUtils.clamp(playerSpeedX, 0, maxSpeed); // nunca negativo no HUD
        ...
        // Botão com ShapeRenderer + texto com BitmapFont
        shapeRenderer.setProjectionMatrix(camera.combined);
        Gdx.gl.glEnable(GL20.GL_BLEND);
        Gdx.gl.glBlendFunc(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
        ...
        if (isPaused) {
            ...
            // Texto centralizado "PAUSADO" e instrução
            ...
            String infoText = "Pressione ESC ou toque no botão para continuar";
